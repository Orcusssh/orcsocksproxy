...
public class SocksKey implements SecretKey {

    private static final long serialVersionUID = 1L;
    private static Logger logger = LoggerFactory.getLogger(SocksKey.class);
    private final static int KEY_LENGTH = 32;
    private byte[] _key;
    private int _length;

    public SocksKey(String password) {
        _length = KEY_LENGTH;
        _key = init(password);
    }

    public SocksKey(String password, int length) {
        _length = length;
        _key = init(password);
    }
